...
await page.getByRole('button',{name:'Turn off calendar',exact:true}).click();
await page.waitForFunction(()=>!document.querySelector('.fc-source-body > .fc-source-error'));
assert.equal(await confirm.isEnabled(),true);assert.equal(policies.at(-1).frequency,'daily');assert.equal(policies.at(-1).operating_calendar.enabled,false);
assert.equal(await frequency.isVisible(),true);assert.equal(await frequency.inputValue(),'daily');
assert.equal(await page.locator('.fc-calendar').count(),1,'Turning the calendar off keeps daily frequency and the editor');
...
console.log('PASS: calendar visibility follows daily/business-daily frequency; turning the calendar off restores the previous policy immediately; incompatible active calendars are disabled; auto detection and legacy frequency-error recovery restore valid monthly data.');await browser.close();
